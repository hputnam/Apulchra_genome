...
There are 8,929,081 CpG sites total in the intergenic region. 
...
There are 8922541 CpG sites total in the intergenic region that have >5x coverage.
...
Bind summary data
```{r}
summary_data_all <- rbind(summary_data_exon, summary_data_intron, summary_data_inter)
```
...
summary_data_meth$methylation_category <- factor(summary_data_meth$methylation_category, levels = c("methylated", "partially methylated"))
...
scale_fill_manual(values = c("methylated" = "#5B85B1FF", "partially methylated" = "#FC766AFF"),
    labels = c("methylated" = "Methylated", "partially methylated" = "Partially methylated")) +
  scale_y_continuous(
...
ggsave("../output/methylation/methylation_feature_percentage.pdf", plot = plot1, width = 8, height = 6, dpi = 300)
ggsave("../output/methylation/methylation_feature_percentage.png", plot = plot1, width = 8, height = 6, dpi = 300)
```

Plot counts
```{r}
# Create the plot
plot2 <- ggplot(summary_data_all, aes(x = type, y = count, fill = methylation_category)) +
  geom_bar(stat = "identity", width = 0.7, color = "black", size = 0.3) +
  scale_fill_manual(values = c("methylated" = "#E64B35", 
                               "partially methylated" = "#4DBBD5", 
                               "unmethylated" = "#00A087"),
                    labels = c("Methylated", "Partially methylated", "Unmethylated")) +
...
# Save the plot
ggsave("../output/methylation/methylation_feature_counts.pdf", plot = plot2, width = 8, height = 6, dpi = 300)
ggsave("../output/methylation/methylation_feature_counts.png", plot = plot2, width = 8, height = 6, dpi = 300)
```
